...
# --- Basic setup: window, clock, colors, fonts ---
pygame.init()
...
# ===================== CLASS 1: Player =====================
class Player:
    # Sets up the player's position, stats, and cooldown timers.
    def __init__(self):
    ...
    # Reads WASD input, moves the player (or continues a dash), and
    # ticks down all active cooldown timers each frame.
    ...
    # Starts the ultimate charge-up if it's off cooldown.
    ...
    # Once fully charged, fires a ring of bullets outward and damages
    # any enemy caught within the blast radius.
    ...
    # Fires a single bullet toward a target point (mouse click), as
    # long as the shoot cooldown has expired.
    ...
    # Starts a short burst dash in whichever direction is currently
    # held, if the dash isn't already on cooldown.
    def dash(self, keys):
    ...
    # Draws the player, plus rings for an active ult charge or
    # portal-power buff.
...
# ===================== CLASS 2: Enemy =====================
class Enemy:
    # Spawns just off one of the four screen edges, with speed/health
    # scaling up as difficulty increases.
    ...
    # Moves straight toward the player each frame.
    ...
    # Deals contact damage to the player if close enough and not on
    # its own hit-cooldown.
    ...
    # Draws a simple layered circle enemy.
    def draw(self, surface):
        pos = (int(self.x), int(self.y))
...
# ===================== CLASS 3: Bullet =====================
class Bullet:
    # A bullet is just a position, a direction, and a "powered" flag
    # (powered bullets from the ult/portal hit harder and faster).
    ...
    # Moves in a straight line and dies once it leaves the screen.
    def update(self, dt):
...
# ===================== CLASS 4: Crystal =====================
class Crystal:
    # A pickup that spawns at a random spot and spins in place.
    def __init__(self):
    ...
    # Heals the player and adds to the score if they walk over it.
    ...
    # Drawn as a rotating diamond shape.
    def draw(self, surface):
...
# ===================== CLASS 5: Portal =====================
class Portal:
    # Appears once the score threshold is hit, and expires on its own
    # after a countdown if never used.
    def __init__(self):
    ...
    # Grants the player a temporary rapid-fire power-up buff on contact.
    def use(self, player):
        if not player.alive: return
        if math.dist((self.x,self.y),(player.x,player.y)) < self.radius + player.radius:
            player.portal_power, self.active = 7, False

    # Drawn as spinning spokes around a ring.
    def draw(self, surface):
        pos = (int(self.x), int(self.y))
...
# ===================== Helper functions =====================
# Draws the dark background with a faint grid overlay.
def draw_background():
...
# Finds the closest enemy to the player, used for the auto-shoot toggle.
...
# Resets every piece of game state back to a fresh run.
def reset_game():
    player = Player()
...
while running:
    ...
    # --- Handle window/keyboard events (pause, dash, ult, restart) ---
    ...
    # --- Core update logic (only runs while playing, not paused/over) ---
    if not game_over and not paused:
        game_time += dt
        ...
        # Manual fire on left click, or auto-fire at the nearest enemy
        # if the auto-shoot toggle is on.
        ...
        # Spawn enemies on a timer that speeds up as the score grows.
        enemy_timer += dt
        ...
        # Spawn a new crystal every 6 seconds.
        crystal_timer += dt
        if crystal_timer >= 6:
            crystals.append(Crystal())
            crystal_timer = 0

        # Spawn the portal once the score threshold is reached.
        if score >= portal_score and portal is None:
            portal = Portal()
            portal_score += 40

        # Move all bullets and drop any that have left the screen.
        for bullet in bullets: bullet.update(dt)
        bullets = [b for b in bullets if b.alive]

        # Move enemies toward the player and apply contact damage.
        for enemy in enemies:
            enemy.update(dt, player, speed_mult)
            enemy.damage_player(player)

        # Check bullet-vs-enemy collisions and award score on kills.
        ...
        # Update the portal and check if the player used it or it expired.
        if portal is not None:
            portal.update(dt)
            portal.use(player)
            if not portal.active: portal = None

        # Game ends once the player dies.
        if not player.alive:
            game_over = True
